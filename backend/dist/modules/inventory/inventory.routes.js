...
router.get('/inventory/stock', requirePermission('inventory.stock.view'), asyncHandler(async (request, response) => {
...
router.get('/inventory/products/:id/movements', requirePermission('inventory.stock.view'), asyncHandler(async (request, response) => {
    const { pageSize, offset } = parsePagination(request.query);
    const items = await stockMovementService.listProductMovements(parseId(request.params.id), { offset, limit: pageSize });
    response.json({ items });
}));
router.post('/inventory/reservations', requirePermission('inventory.reservations.manage'), asyncHandler(async (request, response) => {
    const reservation = await stockReservationService.createReservation(reservationCreateSchema.parse(request.body), request.currentUser.id, request.ip);
    response.status(201).json({ reservation });
}));
router.post('/inventory/reservations/:id/consume', requirePermission('inventory.reservations.manage'), asyncHandler(async (request, response) => {
...
router.post('/inventory/reservations/:id/release', requirePermission('inventory.reservations.manage'), asyncHandler(async (request, response) => {
...
router.post('/inventory/adjustments', requirePermission('inventory.stock.adjust'), asyncHandler(async (request, response) => {
    const adjustment = await stockAdjustmentService.createAdjustment(adjustmentCreateSchema.parse(request.body), request.currentUser.id, request.ip);
    response.status(201).json({ adjustment });
}));
router.post('/inventory/purchases', requirePermission('inventory.purchases.manage'), asyncHandler(async (request, response) => {
...
router.post('/inventory/purchases/:id/receive', requirePermission('inventory.purchases.manage'), asyncHandler(async (request, response) => {
