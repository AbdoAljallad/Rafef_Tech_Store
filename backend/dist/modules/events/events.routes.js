...
router.get('/events/ticker', requirePermission('events.view'), asyncHandler(async (_request, response) => {
    const [rows] = await pool.query(`SELECT id, module, title, message, entity_type, entity_id, severity, created_at
       FROM app_events
