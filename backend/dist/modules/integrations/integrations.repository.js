...
export class IntegrationsRepository {
    ...
    async createOutbox(input) {
        const [result] = await pool.execute(`INSERT INTO integration_webhook_outbox (target, webhook_url, event_type, payload_json, created_by_user_id)
       VALUES (?, ?, ?, CAST(? AS JSON), ?)`, [
            input.target,
    ...
    async claimPending(limit = 10) {
        const [rows] = await pool.query(`SELECT id, target, webhook_url, event_type, payload_json, attempts
       FROM integration_webhook_outbox
       ...
       LIMIT ${Math.max(1, Math.min(limit, 50))}`);
        if (rows.length === 0)
            return [];
        await pool.query(`UPDATE integration_webhook_outbox
       SET status = 'processing'
       WHERE id IN (${rows.map(() => '?').join(',')})`, rows.map((row) => row.id));
        return rows;
    }
    async markSent(id) {
        await pool.execute(`UPDATE integration_webhook_outbox
    ...
    async markFailed(id, error) {
       ...
       WHERE id = ?`, [error.slice(0, 2000), id]);
    }
}
