...
router.patch('/products/:id', requirePermission('catalog.products.manage'), asyncHandler(async (request, response) => {
    const product = await catalogService.updateProduct(parseId(request.params.id), productUpdateSchema.parse(request.body), request.currentUser.id, request.ip);
    response.json({ product });
}));
router.post('/products/:id/price-change', requirePermission('catalog.prices.change'), asyncHandler(async (request, response) => {
    const product = await catalogService.changePrice(parseId(request.params.id), priceChangeSchema.parse(request.body), request.currentUser.id, request.ip);
    response.json({ product });
}));
...
router.get('/categories', requirePermission('catalog.products.view'), asyncHandler(async (_request, response) => {
