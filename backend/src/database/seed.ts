...
async function upsertRole(code: string, nameRu: string, description: string) {
  await pool.execute(
    `INSERT INTO auth_roles (code, name_ru, description, is_system, is_active)
     VALUES (?, ?, ?, TRUE, TRUE)
     ON DUPLICATE KEY UPDATE name_ru = VALUES(name_ru), description = VALUES(description), is_active = TRUE`,
    [code, nameRu, description],
  );
}

async function getRoleId(code: string) {
  const [rows] = await pool.execute<Array<RowDataPacket & { id: number }>>('SELECT id FROM auth_roles WHERE code = ?', [
    code,
  ]);
  const role = rows[0];
  if (!role) {
...
async function seedPermissions() {
  for (const permission of permissions) {
    await pool.execute(
      `INSERT INTO auth_permissions (code, module, name_ru, description)
       VALUES (?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE module = VALUES(module), name_ru = VALUES(name_ru), description = VALUES(description)`,
      [permission.code, permission.module, permission.nameRu, permission.description ?? null],
    );
  }
}

async function assignRoleDefaults(roleCode: string, allowedCodes: string[]) {
  const roleId = await getRoleId(roleCode);
  const [permissionRows] = await pool.query<Array<RowDataPacket & { id: number; code: string }>>(
    'SELECT id, code FROM auth_permissions',
  );
  const allowedSet = new Set(allowedCodes);

  for (const permission of permissionRows) {
    await pool.execute(
      `INSERT INTO auth_role_permission_defaults (role_id, permission_id, is_allowed)
       VALUES (?, ?, ?)
       ON DUPLICATE KEY UPDATE is_allowed = VALUES(is_allowed)`,
      [roleId, permission.id, allowedSet.has(permission.code)],
    );
  }
}
...
async function seedAdminUser() {
  ...
  await pool.execute(
    `INSERT INTO auth_users (role_id, username, password_hash, display_name, status, max_discount_percent)
     VALUES (?, ?, ?, ?, 'active', 100)
     ON DUPLICATE KEY UPDATE role_id = VALUES(role_id), display_name = VALUES(display_name), status = 'active'`,
    [roleId, env.SEED_ADMIN_USERNAME, passwordHash, 'Администратор'],
  );

  const [users] = await pool.execute<Array<RowDataPacket & { id: number }>>(
  ...
  const [permissionRows] = await pool.query<Array<RowDataPacket & { id: number }>>('SELECT id FROM auth_permissions');
  for (const permission of permissionRows) {
    await pool.execute(
      `INSERT INTO auth_user_permissions (user_id, permission_id, is_allowed)
       VALUES (?, ?, TRUE)
...
async function seedAiAssistantUser() {
  ...
  const passwordHash = await bcrypt.hash(
    hasConfiguredPassword ? env.SEED_AI_ASSISTANT_PASSWORD : `disabled-${Date.now()}`,
    12,
  );
  ...
  await pool.execute(
    `INSERT INTO auth_users (role_id, username, password_hash, display_name, status, max_discount_percent)
     ...
     ON DUPLICATE KEY UPDATE role_id = VALUES(role_id), password_hash = VALUES(password_hash), display_name = VALUES(display_name), status = VALUES(status)`,
    [roleId, passwordHash, status],
  );
  ...
  const [permissionRows] = await pool.execute<Array<RowDataPacket & { id: number; code: string }>>(
    `SELECT id, code FROM auth_permissions WHERE code IN ('ai.assistant.use', 'events.view', 'integrations.view')`,
  );
  for (const permission of permissionRows) {
    await pool.execute(
      `INSERT INTO auth_user_permissions (user_id, permission_id, is_allowed)
...
async function seedUnits() {
  ...
  for (const unit of units) {
    await pool.execute(
      `INSERT INTO catalog_units (code, name_ru, allows_fraction)
       VALUES (?, ?, ?)
       ON DUPLICATE KEY UPDATE name_ru = VALUES(name_ru), allows_fraction = VALUES(allows_fraction)`,
      unit,
    );
  }
}
...
async function seedServiceCategoriesAndServices() {
  ...
  const [repairCategories] = await pool.execute<Array<RowDataPacket & { id: number; code: string }>>(
    'SELECT id, code FROM catalog_service_categories WHERE code IN ("repair_labor", "printer_service")',
  );
  const categoryIdByCode = new Map(repairCategories.map((row) => [row.code, row.id]));
  const services: Array<[number | undefined, string, string, string, number]> = [
  ...
  for (const service of services) {
    ...
    const serviceValues: Array<string | number> = [serviceCategoryId, code, defaultName, module, defaultPrice];
    await pool.execute(
...
async function seedRepairDictionaries() {
  ...
  for (const category of categories) {
    await pool.execute(
      `INSERT INTO repair_device_categories (code, name_ru)
       VALUES (?, ?)
       ON DUPLICATE KEY UPDATE name_ru = VALUES(name_ru), is_active = TRUE`,
      category,
    );
  }
  ...
  for (const brand of ['Apple', 'Samsung', 'HP', 'Dell', 'Lenovo', 'Canon', 'Epson']) {
    await pool.execute(
      `INSERT INTO repair_device_brands (name)
       VALUES (?)
...
async function seedProjectTypes() {
  ...
  for (const projectType of projectTypes) {
    await pool.execute(
      `INSERT INTO project_types (code, default_name, description)
       VALUES (?, ?, ?)
       ON DUPLICATE KEY UPDATE default_name = VALUES(default_name), description = VALUES(description), is_active = TRUE`,
      projectType,
    );
  }
}
