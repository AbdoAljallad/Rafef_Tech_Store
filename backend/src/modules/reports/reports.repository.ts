...
export type ReportRow = Record<string, ReportMetricValue>;

export type DetailedReportPayload = {
  report: ReportRow;
  [key: string]: ReportMetricValue | ReportRow[] | ReportRow | null | undefined;
};
...
function buildDateFilter(column: string, filters: ReportFilters, params: SqlParams, prefix: string) {
...
async function one<T extends RowDataPacket>(sql: string, params: SqlParams = {}) {
  const [rows] = await pool.query<T[]>(sql, params);
  return (rows[0] ?? {}) as T;
}

async function many<T extends RowDataPacket>(sql: string, params: SqlParams = {}) {
  const [rows] = await pool.query<T[]>(sql, params);
  return rows;
}

function toReportRow(row: RowDataPacket) {
  return Object.fromEntries(Object.entries(row)) as ReportRow;
}
...
export class ReportsRepository {
  ...
  async finance(filters: ReportFilters): Promise<DetailedReportPayload> {
    const txParams: SqlParams = {};
    const expenseParams: SqlParams = {};
    const refundParams: SqlParams = {};
