...
export class AdminRepository {
  ...
  async getRoleDefaultPermissionIds(roleId: number) {
    const [rows] = await pool.execute<Array<RowDataPacket & { permission_id: number }>>(
      `SELECT permission_id
       FROM auth_role_permission_defaults
       WHERE role_id = ? AND is_allowed = TRUE`,
      [roleId],
    );
  ...
  async replaceUserPermissions(userId: number, permissionIds: number[]) {
  ...
  async applyRoleDefaultPermissions(userId: number, roleId: number) {
  ...
  async permissions() {
    const [rows] = await pool.query<RowDataPacket[]>('SELECT id, code, module, name_ru FROM auth_permissions ORDER BY module, code');
    return rows;
  }
