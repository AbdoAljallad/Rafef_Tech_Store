...
router.post('/finance/accounts', requirePermission('finance.accounts.manage'), asyncHandler(async (req, res) => res.status(201).json({ account: await svc.createAccount(paymentAccountCreateSchema.parse(req.body)) })));
...
router.post('/finance/payment-methods', requirePermission('finance.accounts.manage'), asyncHandler(async (req, res) => res.status(201).json({ method: await svc.createMethod(paymentMethodCreateSchema.parse(req.body)) })));

router.post('/finance/transactions', requirePermission('finance.payments.create'), asyncHandler(async (req, res) => res.status(201).json({ transaction: await svc.createTransaction(transactionCreateSchema.parse(req.body)) })));
...
router.post('/finance/expenses', requirePermission('finance.expenses.manage'), asyncHandler(async (req, res) => res.status(201).json({ expense: await svc.createExpense(expenseCreateSchema.parse(req.body)) })));

router.post('/finance/refunds', requirePermission('finance.refunds.approve'), asyncHandler(async (req, res) => res.status(201).json({ refund: await svc.createRefund(refundCreateSchema.parse(req.body)) })));
...
router.post('/finance/daily-closings', requirePermission('finance.daily_closing.manage'), asyncHandler(async (req, res) => res.status(201).json({ closing: await svc.createDailyClosing(dailyClosingCreateSchema.parse(req.body)) })));
